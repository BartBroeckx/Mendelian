...
{ # general prints
  ...
  if (length(id) > 0){
    q <-as.data.frame(str_split_fixed(Freq[id,1]," ",3), stringsAsFactors=FALSE) 
    q <-cbind(q,Freq[id,2])
...
{ # first some general remarks
  ...
  if (length(id) > 0){
    q <-Freq[id,"output"]
    r <-as.data.frame(str_split_fixed(variants[,1]," / ",2), stringsAsFactors=FALSE) 
    
    id <- which(r[,2] %in% q)
    r <- r[id,]
    r<-cbind(as.data.frame(str_split_fixed(r[,1]," ",3), stringsAsFactors=FALSE),r[,2]) 
